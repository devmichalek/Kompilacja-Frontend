...
- Jeśli stan s zawiera **jakikolwiek** element w formie ```A -> α·Xβ```, gdzie ```X``` jest symbolem terminalnym oraz gdy token z wejścia równy jest terminalowi ```X```, token oraz numer stanu zawierający ten element wrzucany jest na stos.
